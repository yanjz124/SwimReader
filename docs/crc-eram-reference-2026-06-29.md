# CRC ERAM Reference — captured 2026-06-29

Source: https://docs.virtualnas.net/crc/eram/

This is a verbatim conversion of the current vNAS CRC ERAM documentation
(superseding the older capture in [crc-eram-reference.md](crc-eram-reference.md)).
...
![An ERAM display](img/eram/eram.png)Fig. 1 - An ERAM display
...
![The Message Composition Area](img/eram/mca.png)Fig. 2 - The Message Composition Area
...
The arrow keys move the cursor around the Preview Area.

`Ctrl`+`Page Down` and `Ctrl`+`Page Up` jumps the cursor to the end or the start of the Preview Area, respectively.

`Ctrl`+`Enter` moves the cursor to the beginning of the next line in the Preview Area.

`Ctrl`+`R` recalls the last command into the Preview Area.

`Esc` clears the Preview and Feedback areas.
...
The MCA cannot be suppressed, but can be repositioned by left-clicking anywhere within its boundaries. Middle-clicking anywhere with the MCA's boundaries opens the MCA menu for configuring the MCA.

![The Message Composition Area menu](img/eram/mca-menu.png)Fig. 3 - The Message Composition Area menu
...
![The Response Area](img/eram/ra.png)Fig. 4 - The Response Area 

The Response Area (RA) displays text outputted by commands. For example, using the `QF` command outputs flight plan details in the RA (Figure 4).
...
![The Response Area menu](img/eram/ra-menu.png)Fig. 5 - The Response Area menu
...
![The Time View](img/eram/time-view.png)Fig. 6 - The Time View
...
![The Time View menu](img/eram/time-view-menu.png)Fig. 7 - The Time View menu
...
![The Beacon Code View](img/eram/beacon.png)Fig. 8 - The Beacon Code View
...
The Beacon Code View can be displayed or suppressed from the **VIEWS** Toolbar menu by left or middle-clicking the **CODE** button. The view can be repositioned by left or middle-clicking the title, and suppressed by left or middle-clicking the suppression button (**-**). Left or middle-clicking the menu button (**M**) displays the Beacon Code View menu for configuring the Beacon Code View.

![The Beacon Code View menu](img/eram/beacon-menu.png)Fig. 9 - The Beacon Code View menu
...
![The Check List View](img/eram/checklist.png)Fig. 10 - The Check List View
...
![The Altimeter Settings View](img/eram/altimeter-list.png)Fig. 12 - The Altimeter Settings View
...
![The Altimeter Settings View menu](img/eram/altimeter-list-menu.png)Fig. 13 - The Altimeter Settings View menu
...
![The Continuous Range Readout View](img/eram/crr-view.png)Fig. 14 - The Continuous Range Readout View
...
![The Continuous Range Readout View menu](img/eram/crr-menu.png)Fig. 15 - The Continuous Range Readout View menu
...
The Continuous Range Readout View menu contains the following options:
  ...
  * **LIST** : toggles the Continuous Range Readout View between the list and panel modes (Figure 16)
...
![The Continuous Range Readout View in panel mode](img/eram/crr-panel.png)Fig. 16 - The Continuous Range Readout View in panel mode 
...
![The Weather Station Report View](img/eram/weather-list.png)Fig. 17 - The Weather Station Report View
...
![The Weather Station Report View menu](img/eram/weather-list-menu.png)Fig. 18 - The Weather Station Report View menu
...
The `WR` command can also be used to display the station's weather report in the Response Area without adding or removing the station from the Weather Station Report View:
...
The `MR` command can also be used to display a list of the available GeoMaps in the Response Area:

`MR`

A GeoMap Filter is toggled by left or middle-clicking the appropriate filter button in the GeoMap Toolbar menu.

Some GeoMap elements may not be assigned to a Filter and will always be visible.

Some GeoMap elements may only be visible in Top-Down mode.

The brightness of GeoMap elements is controlled from the **MAP BRIGHT** Toolbar menu. Note that the Brightness Control Groups (BCGs) do not necessarily directly correspond to GeoMap Filters.

When disconnected from the VATSIM network, the message **NO RADAR DATA** is displayed in the center of the display.
...
Table 1 - Targets Name | Symbol | Description  
---|---|---  
...
![A Target with a history trail](img/eram/targets/history.png)Fig. 19 - A Target with a history trail

The number of displayed Target history data points is controlled by the **HISTORY** button on the **RADAR FILTER** Toolbar menu. Each history point displays the Target's symbol from when it was at that location.
...
Table 2 - Tracks and Position Symbols Name | Position Symbol | Description  
---|---|---  
...
If the location is near the aircraft's Target and the aircraft's flight plan has been activated with a `DM` command, the created Track will associate with the Target. However, Tracks do not need to be created near a Target, nor does the specified aircraft ID have to match an existing flight plan.
...
Handoffs are initiated by an Implied command with the receiving sector's ID:

`<sector ID> <FLID>`

Sector IDs can be found in the [Controller List](<../overview/#controller-list>).

Pending handoffs are accepted or recalled by an Implied command with the aircraft's FLID:

`<FLID>`

Unlike legacy clients, handoffs cannot be rejected.
...
After a point out is initiated, an aircraft's FDB cannot be changed to an LDB with the standard `<FLID>` Implied command, even if the point out has been accepted and cleared. The `QP` command must be used to change the FDB to an LDB:

`QP <FLID>`

Unlike legacy clients, point outs can only be initiated to other vNAS ERAM sectors.

![An FDB with a pending point out](img/eram/pointout.png)Fig. 20 - An FDB with a pending point out

When a point out is initiated, the initiating sector's FDB displays a yellow **P** indicator on line 0. When the point out is acknowledged, the **P** indicator changes to a white **A** indicator. Left or middle-clicking the **A** indicator removes it from the FDB.

![A point out menu with a pending point out](img/eram/pointout-menu.png)Fig. 21 - A point out menu with a pending point out
...
![A point out menu with an unacknowledged point out](img/eram/pointout-receive-menu.png)Fig. 22 - A point out menu with an unacknowledged point out
...
The point out pop-up menu can be repositioned by left or middle-clicking the title, and closed by left or middle-clicking the close button (**X**).

### Freezing a Track

![A frozen Track](img/eram/frozen.png)Fig. 23 - A frozen Track
...
A frozen Track is moved to and remains at the specified location and will not coast. A frozen Track's Data Block displays **FRZN** in Field E.

A Track is unfrozen by starting track on the frozen Track.
...
`LA [<location 1> | <track>] <location 2> T`
...
Limited Data Blocks (LDBs) have no track position symbol, leader line, or velocity vector. An LDB may only be positioned to the left or right of its Target.

#### Unpaired LDBs

![An unpaired LDB](img/eram/data-blocks/unpaired-ldb.png)Fig. 24 - An unpaired LDB

An unpaired LDB displays its Target's beacon code and altitude of hundreds of feet MSL. If the Target is within the ARTCC's ADS-B AOI, the Target's aircraft ID can be displayed in place of its beacon code by activating the **BCAST FLID** option in the **DB FIELDS** Toolbar menu or converting the LDB to an E-LDB.

#### Paired LDBs

![A paired LDB](img/eram/data-blocks/paired-ldb.png)Fig. 25 - A paired LDB

A paired LDB display its Track's aircraft ID and altitude in hundreds of feet MSL.

#### Enhanced LDBs

![An enhanced LDB](img/eram/data-blocks/eldb.png)Fig. 26 - An enhanced LDB

An LDB can be converted to or from an Enhanced LDB (E-LDB) by left or middle-clicking the altitude field. If the **BCAST FLID** option is inactive, an unpaired E-LDB displays its Target's aircraft ID in place of its beacon code. Both paired and unpaired E-LDBs display the same altitude information as the second line of an FDB. Left or middle-clicking the second line of an E-LDB returns it to its non-enhanced format.

### Full Data Blocks

![An FDB](img/eram/data-blocks/fdb.png)Fig. 27 - An FDB

Full Data Blocks (FDBs) have a track position symbol, leader line, and velocity vector lines. An FDB may be positioned at any of the eight positions adjacent to its Target as listed in Table 6.

![FDB fields](img/eram/data-blocks/fdb-fields.png)Fig. 28 - FDB fields
...
![VCI](img/eram/data-blocks/vci.png)Fig. 29 - VCI

  * **Visual Communications Indicator (VCI)** : a symbol indicating the aircraft has checked in on frequency. The VCI is toggled by left or middle-clicking the symbol's location, or by the `//` Implied command:
...
FDB line 0 displays the following item:
  ...
  * **Point Out Indicator** : a **P** indicating a point out to/from another sector has been initiated, or an **A** indicating a point out has been acknowledged by another sector. For more information on point outs, please see the point outs section of the documentation.
...
FDB line 1 contains the following fields:
  ...
  * **Field A** : the aircraft's ID. Left-clicking Field A toggles Dwell Emphasis lock. Middle-clicking Field A executes the `QF` command.
...
Table 3 - Field B character 4 values Character | Fields B and C Example | Description  
---|---|---  
...
![An FDB with a non-RVSM indicator](img/eram/data-blocks/non-rvsm.png)Fig. 30 - An FDB with a non-RVSM indicator
...
Table 4 - Field E values Data | Description  
---|---  
...
**Hxxx** | The Track is being handed off to sector xxx  
...
**FRZN** | The data block is unpaired from the Target and frozen  
...
Table 5 - The `QS` command Command | Description  
---|---  
...
Velocity vector line length is controlled from the **VECTOR** button on the Master Toolbar. Velocity vector lines can be set to extend 0 (hidden), 1, 2, 4, or 8 minutes.

`Page Up` and `Page Down` increases and decreases the length of velocity vector lines, respectively.
...
Unlike legacy clients, velocity vector line length is synced between all controllers on a sector.

#### Distance Reference Indicators

![A standard and reduced separation DRI](img/eram/data-blocks/dris.png)Fig. 31 - A standard and reduced separation DRI
...
![An Altitude Menu](img/eram/data-blocks/altitude-menu.png)Fig. 32 - An Altitude Menu
...
Assigned Altitudes in non-RVSM airspace are displayed in gray if the aircraft is not RVSM equipped.
...
![A Heading Menu](img/eram/data-blocks/heading-menu.png)Fig. 33 - A Heading Menu
...
![A Speed Menu](img/eram/data-blocks/speed-menu.png)Fig. 34 - A Speed Menu
...
![A Free Form Text Box Menu](img/eram/data-blocks/free-form-text-box-menu.png)Fig. 35 - A Free Form Text Box Menu
...
The Free Form Text Box Menu cannot be opened if the aircraft does not have any assigned fourth line free form text.

### Range Data Blocks

![An RDB](img/eram/data-blocks/rdb.png)Fig. 36 - An RDB

Range Data Blocks (RDBs) display the aircraft's straight-line distance to a CRR group location in nautical miles. The are only displayed in addition to an FDB. For more information on CRR groups, please see the LF command section of the documentation.

`Ctrl`+`Shift`+`O` cycles the offset between the RDB and the FDB's leader line.

### Conflict Data Blocks

![A CDB](img/eram/data-blocks/cdb.png)Fig. 37 - A CDB

When a tracked flight is in conflict with a Mode C intruder, the system displays a Conflict Data BLock (CDB) for the Mode C intruder. A CDB displays a Track position symbol, leader line, and velocity vector lines and is automatically positioned relative its Target based on the Target's velocity direction.
...
To cycle between an LDB and an E-LDB, left or middle-click the altitude field.

After a point out is initiated, an aircraft's FDB cannot be minimized with the standard `FLID` Implied command, even if the point out has been accepted and cleared. To minimize the FDB, the `QP` command must be used:

`QP <FLID>`

### Positioning Data Blocks

A Data Block can be positioned relative to its Target or Track by an Implied command with one of the positions listed in Table 6:

`<1-9> <FLID>`

Table 6 - Data Block positions  Input | Data Block Position  
---|---  
1 | SW  
2 | S  
3 | SE  
...
The Data Block position and leader line length Implied command can be combined into a single command:

`<1-9>/<0-3> <FLID>`

### Dwell Emphasis

![A Track with Dwell Emphasis](img/eram/data-blocks/dwell.png)Fig. 38 - A Track with Dwell Emphasis

A Data Block is temporarily displayed with Dwell Emphasis when the cursor is above the Data Block or the Data Block's Track symbol. Dwell Emphasis lock is toggled by left-clicking field A.
...
![A QF command response](img/eram/ra.png)Fig. 39 - A `QF` command response

The `QF` command displays a readout of the flight plan's contents in the Response Area:
...
Unlike legacy clients, `QB V`, `QB R`, and `QB T` no longer assign a voice type to an aircraft. Instead, the voice type parameter must be prefixed with a `/`.
...
`QZ VFR <FLID>` or `QZ VFR/<altitude> <FLID>` or `QZ OTP <FLID>`
...
Instead of `/OK`, `/TT` or `///` is used in the `QQ` command to override logic checks: `QQ /TT P150 JBU123`

Multiple FLIDs may be specified in one `QQ` command: `QQ 110 JBU123/429/AAL924`
...
For more information on how altitudes are displayed in Data Blocks, please see the Data Blocks section of the documentation.

### Routing

##### QU Command

![A route display](img/eram/route.png)Fig. 40 - A route display
...
Multiple FLIDs may be specified in one `QU` command: `QU 30 JBU123/429/AAL924`

Unlike legacy clients, route displays are displayed to all controllers logged in to the same sector.

The QU command is also used to amend an aircraft's route direct to one or more specified fix or location:

`QU [<fix> | <location>] <FLID>`
...
Logic checks may be overridden with `/OK`: `QU /OK PROVI JBU123`

Locations can be entered as left-clicked display locations. This makes it easy to draw a path around weather when rerouting an aircraft.

##### AM Command

The `AM` command is used to amend portions of a flight plan. It is used as follows:

Table 7 - The `AM` command Command | Field Number | Description | Example  
---|---|---|---  
...
The Field Number may be used in place of the field name, such as `AM <FLID> 3 B737/L`.
...
Table 8 - The `AM RTE` command Command | Description | Resulting Route  
---|---|---  
...
Table 9 - A Menu Button Closed Menu Button | Open Menu Button  
---|---  
...
Table 10 - A Toggle Button Deactivated Toggle Button | Activated Toggle Button  
---|---  
...
![An Increment/Decrement Button](img/eram/toolbars/buttons/inc-dec.png)Fig. 41 - An Increment/Decrement Button
...
Table 11 - A Command Button Deactivated Command Button | Activated Command Button  
---|---  
...
Many Toolbar buttons support being "torn-off" from their toolbar. A torn-off button can be placed anywhere on the ERAM display and remains visible when its Toolbar is suppressed. A button is torn-off by left or middle-clicking the gold tear-off bar on the left side of the button. Only one tear-off can be made of each button. A torn-off button can be repositioned similarly to a view by left or middle-clicking its gold tear-off bar. To delete a tear-off, left or middle-click the **DELETE TEAROFF** button on the Master Toolbar, then left-click to select or deselect tear-offs. Middle click to confirm deletion of selected tear-offs.

### Master Toolbar

The ERAM Master Toolbar contains buttons for controlling various ERAM system settings and functions. Some Toolbar buttons toggle the display of additional toolbars. The Master Toolbar is displayed by left or middle-clicking the **MASTER TOOLBAR** button in the Toolbar Control Menu. The Toolbar Control Menu's tear-off button is always displayed.

### Toolbar Control Menu

![The Toolbar Control menu](img/eram/toolbars/control.png)Fig. 42 - The Toolbar Control menu
...
![The Master Toolbar menu](img/eram/toolbars/main.png)Fig. 43 - The Master Toolbar menu

The Master Toolbar menu contains the following buttons:

  * **DRAW** : not simulated
  * **ATC TOOLS** : opens the ATC Tools Toolbar menu
  * **RANGE** : controls the range of the display. The display's vertical range in hundreds of NMs is displayed.
  * **CURSOR** : opens the Cursor Toolbar menu
  * **BRIGHT** : opens the Brightness Toolbar menu
  * **FONT** : opens the Font Toolbar menu
  * **DB FIELDS** : opens the Data Block Fields Toolbar menu
  * **VECTOR** : controls the length of data block velocity vectors
  * **VIEWS** : opens the Views Toolbar menu
  * **CHECK LISTS** : opens the Check Lists Toolbar menu
  * **COMMAND MENUS** : not simulated
  * **GeoMap** : opens the GeoMap Toolbar menu. The current GeoMap's name is displayed.
  ...
  * **RADAR FILTER** : opens the Radar Filter Toolbar menu
  * **PREFSET** : not simulated
  * **DELETE TEAROFF** : initiates the delete tear-off function


### Views Toolbar Menu

![The Views Toolbar menu](img/eram/toolbars/views.png)Fig. 44 - The Views Toolbar menu

The Views Toolbar menu contains the following buttons:

  * **ALTIM SET** : toggles display of the Altimeter Settings View
  * **AUTO HO INHIB** : not simulated
  * **CFR** : not simulated
  * **CODE** : toggles display of the Beacon Code View
  ...
  * **CRR** : toggles display of the Continuous Range Readout View
  ...
  * **UA** : not simulated
  * **WX REPORT** : toggles display of the Weather Station Report View


### ATC Tools Toolbar Menu

![The ATC Tools Toolbar menu](img/eram/toolbars/atc-tools.png)Fig. 45 - The ATC Tools Toolbar menu

The ATC tools Toolbar menu contains the following buttons:

  * **CRR FIX** : displays CRR fixes
  * **SPEED ADVSRY** : not simulated
  * **WX** : opens the Weather Toolbar menu


### Weather Toolbar Menu

![The Weather Toolbar Menu](img/eram/toolbars/weather.png)Fig. 46 - The Weather Toolbar Menu

The weather Toolbar menu contains the following buttons:

  * **NX Altitudes** : not simulated
  * **NX LVL** : controls the displayed NEXRAD precipitation levels
  * **WX1** : not simulated
  * **WX2** : not simulated
  * **WX3** : not simulated
...
![The Check Lists Toolbar menu](img/eram/toolbars/check-lists.png)Fig. 47 - The Check Lists Toolbar menu

The Check Lists Toolbar menu contains the following buttons:

  * **POS CHECK** : toggles display of the position relief Check List View
  * **EMERG CHECK** : toggles display of the emergency Check List View


### Cursor Toolbar Menu

![The Cursor Toolbar menu](img/eram/toolbars/cursor.png)Fig. 48 - The Cursor Toolbar menu
...
![An example GeoMap Toolbar menu](img/eram/toolbars/geomap.png)Fig. 49 - An example GeoMap Toolbar menu

The GeoMap Toolbar menus contains a Toggle Button for each GeoMap Filter. Holding `Alt` displays an unrealistic set of an additional 20 Filters.

### Brightness Toolbar Menu

The Brightness Toolbar menu contains the following buttons:

  * **MAP BRIGHT** : opens the Map Brightness Toolbar menu
  * **CPDLC** : not simulated
  ...
  * **PR TGTR** : controls the brightness of paired Targets
  * **UNP TGT** : controls the brightness of unpaired Targets
  * **PR HIST** : controls the brightness of paired Target history data points
  * **UNP HIST** : controls the brightness of unpaired Target history data points
  * **LDB** : controls the brightness of LDBs
  * **SLDB** : adjusts the brightness of selected LDBs
  * **WX** : not simulated
  * **NEXRAD** : controls the brightness of NEXRAD weather
  ...
  * **FDB** : controls the brightness of FDBs
  * **PORTAL** : adjusts the brightness of elements outside of an FDB's Portal Fence
  * **SATCOMM** : not simulated
  * **ON-FREQ** : controls the brightness of on-frequency indicators (VCIs)
  * **LINE 4** : adjusts the brightness of FDB fourth-line data
  * **DWELL** : adjusts the brightness of Dwelled Data Blocks and Track symbols
...
![An example Map Brightness Toolbar menu](img/eram/toolbars/map-brightness.png)Fig. 50 - An example Map Brightness Toolbar menu
...
![The Font Toolbar menu](img/eram/toolbars/font.png)Fig. 51 - The Font Toolbar menu
...
![The Radar Filter Toolbar menu](img/eram/toolbars/radar-filter.png)Fig. 52 - The Radar Filter Toolbar menu
...
The Radar Filter Toolbar menu contains the following Toggle Buttons:
  ...
  * **SELECT BEACON** : toggles display of Targets for aircraft squawking a code in the Beacon Code View
...
![The Data Block Fields Toolbar menu](img/eram/toolbars/db-fields.png)Fig. 53 - The Data Block Fields Toolbar menu
...
The Data Block Fields Toolbar menu contains the following buttons:
  ...
  * **FDB LDR** : controls the length of FDB leader lines
...
`QD <lower limit>B<upper limit>`

## NEXRAD

![Hurricane Idalia depicted in NEXRAD](img/eram/nexrad/nexrad.png)Fig. 54 - Hurricane Idalia depicted in NEXRAD
...
Table 12 - NEXRAD precipitation levels Moderate Precipitation | Heavy Precipitation | Extreme Precipitation  
---|---|---  
...
To remove lower levels of precipitation from display, left-click the **NX LVL** button in the **WX** Toolbar menu. Middle-click the button to show lower levels of precipitation. Precipitation levels are shown as follows:

Table 13 - NEXRAD level settings NX LVL setting | Displayed Precipitation  
---|---  
OFF | None  
3 | Extreme  
...
vNAS supports the ability to automatically begin tracking aircraft upon departure from a customizable list of airports. When an aircraft departs an airport on the autotrack list, a `DM` command is automatically executed, followed by a `QT` command.
...
Table 14 - Scope markers commands Command | Description | BCG(s) | Font Size  
---|---|---|---  
...
For the purposes of VATSIM, ERAM Top-Down mode (TDM) can be toggled with `Ctrl`+`T`. In Top-Down mode, TDM-only GeoMap elements (such as airport diagrams) and Ground Targets are displayed. There are two types of Ground Targets displayed in Top-Down mode:

Table 15 - Ground Targets Aircraft | Heavy Aircraft  
---|---  
...
![A Ground Target and Data Block with a correlated flight plan](img/eram/ground-data-block.png)Fig. 55 - A Ground Target and Data Block with a correlated flight plan
...
Ground Target Data Blocks are repositioned by simply left-clicking and dragging to a desired position. While in TDM, Data Blocks may also be positioned and leader line lengths may be set similarly to standard Data Blocks.
...
Table 16 - Implied commands Command | Description | Supports `/OK` | Supports multiple FLIDs  
---|---|---|---  
`<FLID>` | Changes an unowned Data Block between an LDB and an FDB |  |   
`<FLID>` | Accepts or recalls a pending handoff | Yes |   
`//<FLID>` or `// <FLID>` | Toggles a Track's VCI |  |   
`<1-9> <FLID>` | Positions a Data Block |  |   
`/<0-3> <FLID>` | Sets Data Block's leader line length |  |   
`<1-9>/<0-3> <FLID>` | Positions a Data Block and sets its leader line length |  |   
  
### ERAM Commands

The following commands are typed into the MCA and executed by pressing `Enter` or middle-clicking to select a track or location (if supported by the command).

Table 17 - ERAM commands Command | Description | Supports `/OK` | Supports multiple FLIDs  
---|---|---|---  
`AM` (see link for full syntax) | Amends a flight plan |  |   
`AR <station ID>` | Toggles display of an altimeter setting |  |   
`DM <ACID>` | Activates an aircraft's flight plan |  |   
`LA`(see link for full syntax) | Displays range/bearing between two locations |  |   
`LB` (see link for full syntax) | Displays range/bearing between fix and location |  |   
`LC`(see link for full syntax) | Displays speed adjustment to position the track over the fix at the specified time |  |   
`LF` (see link for full syntax) | Creates a CRR group, or adds or removes aircraft to/from a CRR group |  | Yes  
`MR` | Displays the list of available GeoMaps in the Response Area |  |   
`MR <GeoMap>` | Switches to the specified GeoMap |  |   
`QB <ACID>` | Requests a beacon code |  |   
`QB <code> <ACID>` | Assigns a beacon code |  |   
`QB <code1> <code2> ...` | Adds/removes beacon codes from the Beacon Code View |  |   
`QB <equipment> <FLID>` | Assigns an equipment suffix |  |   
`QB [/v\|/r\|/t] <FLID>` | Assigns a voice type |  |   
`QD <station ID>` | Toggles display of an altimeter setting |  |   
`QD <lower limit>B<upper limit>` | Sets altitude limits |  |   
`QF` | Displays flight plan readout in the Response Area |  |   
`QH F <location> <FLID>` | Freezes a Track |  |   
`QN` or `QZ` | Executes an Implied command. Entering `QN` or `QZ` is not required to execute an Implied command. |  |   
`QP <sector> <FLID>` | Initiates a point out |  |   
`QP A <sector> <FLID>` | Acknowledges a point out |  |   
`QP <FLID>` | Changes an FDB to an LDB after a point out |  |   
`QP J <FLID>` | Toggles display of a standard DRI |  |   
`QP T <FLID>` | Toggles display of a reduced separation DRI for an eligible Track |  |   
`QQ (R\|P\|L)<altitude> <FLID>` | Sets an interim altitude, local interim altitude, or procedure altitude | Yes, use `///` or `/TT` | Yes  
`QR <altitude> <FLID>` | Sets a controller entered reported altitude |  |   
`QS` (see link for full syntax) | Edits FDB HSF fourth line data |  |   
`QT <location> <ACID>` | Starts a track |  |   
`QU <minutes> <FLID>` | Displays a route | Yes |   
`QU <fix> <FLID>` | Amends a route direct to a fix |  |   
`QX <FLID>` | Drops a Track | Yes |   
`QZ <altitude> <FLID>` | Sets an assigned altitude |  |   
`RF <FLID> <facility>` | Forces a flight plan transfer to the specified facility |  |   
`VP <aircraft type> <route> <FLID>` | Creates a VFR flight plan |  |   
`WR <station ID>` | Toggles display of a weather station report |  |   
`WR R <station ID>` | Displays the most recent report from the weather station in the Response Area |  |   
  
### Keyboard Shortcuts

Table 18 - Keyboard shortcuts Keyboard Shortcut | Function  
---|---  
...
`Ctrl`+`T` | Toggles Top-Down mode  
`Ctrl`+`Del` | Clears the Response Area  
`Ctrl`+`Shift`+`B` | Resets the default brightness settings  
`Ctrl`+`Shift`+`O` | Cycles the RDB offset  
  
## Settings

![ERAM display settings](img/eram/settings.png)Fig. 56 - ERAM display settings
...
Table 19 - ERAM acronyms and abbreviations Term | Definition  
---|---  
