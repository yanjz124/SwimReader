...
**RadarWindow.cs port progress: lines 1542 / 6962** — `ProcessCommand` (source 1548-2879, ~1332 lines, one un-splittable switch) is being translated in `.wip/ProcessCommand.wip.js` and spliced in once complete. Part 1 done (method head → `'+'` case, ~source 2062); remaining: `F7`/ATPA multifunction onward. Key shim gained function-key Symbols (char=string vs named-key=Symbol contract, so `GetType()==typeof(char)` → `typeof x==="string"`).
